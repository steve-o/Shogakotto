...
// no confirmation on close
		...
		public void resubmit() {
			if (this.pending_connection) {
				LOG.trace ("Cannot resubmit due to pending connection.");
				return;
			}
...
/* TBD: snapshots, a.k.a. non-streaming requests not always supported */
			msg.setIndicationFlags (OMMMsg.Indication.REFRESH | OMMMsg.Indication.PRIVATE_STREAM);
...
/* OMMAttribInfo.Attrib as an OMMFieldList */
			this.omm_encoder.initialize (OMMTypes.MSG, OMM_PAYLOAD_SIZE);
...
/* Forward all MMT_ANALYTICS encapsulated messages */
		...
		private void OnAnalyticsStatus (ResponseStatus response_status, AnalyticStream stream) {
			LOG.trace ("ResponseStatus: {}", response_status);
		}
...
/* Construct directory unique key */
	...
	public void createAnalyticStream (Analytic analytic, AnalyticStream analytic_stream) {
		analytic_stream.setQuery (analytic.getQuery());
		analytic_stream.setItemName (analytic.getItem());
		analytic_stream.setAppName (analytic.getApp());
		analytic_stream.setServiceName (analytic.getService());
...
/* TBD: no stream de-duplication */
		app.createItemStream (analytic_stream);
...
//GenericOMMParser.parse (msg);
		...
		case OMMState.Stream.CLOSED_RECOVER:
			LOG.trace ("Pending login recovery.");
			break;
